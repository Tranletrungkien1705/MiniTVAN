...
// CQT phản hồi thông điệp loại 301 xác nhận đã tiếp nhận/xử lý thông báo 300 → FlagSuaDoi = Allowed.
app.MapPost("/api/invoices/{id:int}/tct-301", async (int id, Tct301Dto dto, ITvanService svc) =>
{
    var (ok, msg) = await svc.ReceiveTct301Async(id, dto.TctRefNo, dto.By);
    return ok ? Results.Ok(new { id, msg }) : Results.BadRequest(new { id, error = msg });
});

// Nhật ký nhận kết quả xử lý thông báo sai sót (301) từ CQT (lọc theo hóa đơn nếu có).
app.MapGet("/api/tct-301-logs", async (int? invoiceId, ITvanService svc) =>
{
    var ls = await svc.Tct301LogsAsync(invoiceId);
    return Results.Ok(ls.Select(l => new { l.Id, l.InvoiceId, invoice = l.Invoice != null ? $"{l.Invoice.Symbol}-{l.Invoice.No}" : null, l.TCTRefNo, flagReplaceOrAdjust = (int)l.FlagReplaceOrAdjust, ketQua = l.KetQua.ToString(), l.Message, l.By, l.CreatedAt }));
});
...
// Danh mục loại dòng hàng hóa/dịch vụ trên hóa đơn (theo Mst_InvoiceDtlType của TVAN gốc): danh sách (lọc theo từ khóa nếu có).
app.MapGet("/api/invoice-dtl-types", async (string? keyword, ITvanService svc) =>
{
    var ls = await svc.InvoiceDtlTypesAsync(keyword);
    return Results.Ok(ls.Select(t => new { t.Id, t.InvoiceDtlTypeCode, t.Desc, t.FlagActive, t.UpdatedAt, t.UpdatedBy }));
});

// Lưu (tạo mới/cập nhật) loại dòng hàng hóa/dịch vụ theo mã (theo Mst_InvoiceDtlType của TVAN gốc).
app.MapPost("/api/invoice-dtl-types", async (InvoiceDtlTypeDto dto, ITvanService svc) =>
{
    var (ok, msg, id) = await svc.SaveInvoiceDtlTypeAsync(dto.Id, dto.Code ?? "", dto.Desc, dto.Active, dto.By);
    return ok ? Results.Ok(new { id, msg }) : Results.BadRequest(new { id, error = msg });
});

// Xóa loại dòng hàng hóa/dịch vụ theo id (theo Mst_InvoiceDtlType của TVAN gốc).
app.MapDelete("/api/invoice-dtl-types/{id:int}", async (int id, ITvanService svc) =>
{
...
// Danh mục nhóm sản phẩm (theo Mst_SpecType2 của TVAN gốc): danh sách (lọc theo từ khóa nếu có).
app.MapGet("/api/spec-type2s", async (string? keyword, ITvanService svc) =>
{
    var ls = await svc.SpecType2sAsync(keyword);
    return Results.Ok(ls.Select(t => new { t.Id, t.SpecType2Code, t.SpecType2Name, t.Remark, t.FlagActive, t.UpdatedAt, t.UpdatedBy }));
});

// Lưu (tạo mới/cập nhật) nhóm sản phẩm theo mã (theo Mst_SpecType2_Create/Update của TVAN gốc).
app.MapPost("/api/spec-type2s", async (SpecType2Dto dto, ITvanService svc) =>
{
    var (ok, msg, id) = await svc.SaveSpecType2Async(dto.Id, dto.Code ?? "", dto.Name ?? "", dto.Remark, dto.Active, dto.By);
    return ok ? Results.Ok(new { id, msg }) : Results.BadRequest(new { id, error = msg });
});

// Xóa nhóm sản phẩm theo id (theo Mst_SpecType2_Delete của TVAN gốc).
app.MapDelete("/api/spec-type2s/{id:int}", async (int id, ITvanService svc) =>
{
...
// Đơn vị quy đổi của sản phẩm (theo Mst_SpecUnit của TVAN gốc): danh sách (lọc theo từ khóa + sản phẩm + đơn vị nếu có).
app.MapGet("/api/spec-units", async (string? keyword, string? specCode, string? unitCode, ITvanService svc) =>
{
    var ls = await svc.SpecUnitsAsync(keyword, specCode, unitCode);
    return Results.Ok(ls.Select(t => new { t.Id, t.SpecCode, t.UnitCode, t.StandardUnitCode, t.SpecUnitDesc, t.Qty, t.Length, t.Width, t.Height, t.Volume, t.Weight, t.Remark, t.FlagActive, t.UpdatedAt, t.UpdatedBy }));
});

// Lưu (tạo mới/cập nhật) đơn vị quy đổi theo cặp (sản phẩm, đơn vị) (theo Mst_SpecUnit_Create/Update của TVAN gốc).
app.MapPost("/api/spec-units", async (SpecUnitDto dto, ITvanService svc) =>
{
    var (ok, msg, id) = await svc.SaveSpecUnitAsync(dto.Id, dto.SpecCode ?? "", dto.UnitCode ?? "", dto.StandardUnitCode ?? "", dto.Desc, dto.Qty, dto.Length, dto.Width, dto.Height, dto.Volume, dto.Weight, dto.Remark, dto.Active, dto.By);
    return ok ? Results.Ok(new { id, msg }) : Results.BadRequest(new { id, error = msg });
});

// Xóa đơn vị quy đổi theo id (theo Mst_SpecUnit_Delete của TVAN gốc).
app.MapDelete("/api/spec-units/{id:int}", async (int id, ITvanService svc) =>
{
...
record Tct301Dto(string? TctRefNo, string? By);
