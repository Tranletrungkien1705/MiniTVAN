...
public class InvoiceController(ITvanService svc) : Controller
{
    ...
    public async Task<IActionResult> Detail(int id)
    {
        ...
        ViewBag.Tct301Logs = await svc.Tct301LogsAsync(id);
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> ReceiveTct301(int id, string? tctRefNo, string? by)
    {
        var (ok, msg) = await svc.ReceiveTct301Async(id, tctRefNo, by);
        TempData[ok ? "Success" : "Error"] = msg;
        return RedirectToAction(nameof(Detail), new { id });
    }
...
public class Tct301LogController(ITvanService svc) : Controller
{
    public async Task<IActionResult> Index(int? invoiceId)
    {
        ViewBag.InvoiceId = invoiceId;
        return View(await svc.Tct301LogsAsync(invoiceId));
    }
}
...
// các loại sản phẩm (nhóm hàng hóa theo loại) dùng để phân loại sản phẩm/hàng hóa khi lập hóa đơn.
public class SpecType1Controller(ITvanService svc) : Controller
{
    public async Task<IActionResult> Index(string? keyword)
    {
        ViewBag.Keyword = keyword;
        return View(await svc.SpecType1sAsync(keyword));
    }

    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Save(int? id, string code, string name, string? remark, bool active, string? by)
    {
        var (ok, msg, _) = await svc.SaveSpecType1Async(id, code, name, remark, active, by);
        TempData[ok ? "Success" : "Error"] = msg;
        return RedirectToAction(nameof(Index));
    }
